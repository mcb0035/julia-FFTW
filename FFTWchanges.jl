...
import DFTchanges: fft, bfft, ifft, rfft, brfft, irfft, plan_fft, plan_bfft, plan_ifft, plan_rfft, plan_brfft, plan_irfft, fft!, bfft!, ifft!, plan_fft!, plan_bfft!, plan_ifft!, Plan, rfft_output_size, brfft_output_size, plan_inv, normalization, ScaledPlan
...
#        println("show tensor:")
#        ccall(("showtensorc99", "/home/qm4/tests/libccall.so"), Void, (Ptr{tensor},), sz)

#=        ccall(("tensor_print", "/home/qm4/tests/libccall.so"),
              Void,
#              (TensPtr,),
              (Ptr{tensor},),
              sz)
        ccall(($(string(fftw,"_tensor_print")),$lib),
                      Void,
                      (TensPtr, PrintPtr),
                      sz, prnt)
=##END TMP
...
#        ccall(("showtensorc99", "/home/qm4/tests/libccall.so"), Void, (Ptr{tensor},), vecsz)
        ...
        if isa(Y, FakeArray)
            ro, io = similar(ri), similar(ii)
        else
            ro, io = extract_reim(Cint(direction), Y)
        end 
#        println("ri: $(typeof(ri))\n$ri")
